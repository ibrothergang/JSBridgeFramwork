...
public class H5PagePlugin implements H5Plugin {
    ...
    public void getFilter(H5IntentFilter filter) {
        ...
        filter.addAction(H5Container.H5_PERFORMANCE);
    }
    ...
    public boolean handleIntent(H5Intent intent) {
        ...
        if (H5Plugin.H5_PAGE_BACK_BEHAVIOR.equals(action)) {
            ...
            if (h5Page != null && h5Page.getSession() != null
                    && h5Page.getSession().getScenario() != null
                    && h5Page.getSession().getScenario().getData() != null) {
            ...
            h5WebView.getUnderlyingWebView().setBackgroundColor(color);
        } else if (H5Plugin.H5_TOOLBAR_MENU_BT.equals(action)) {
            String tag = H5Utils.getString(param, H5Container.MENU_TAG);
        ...
        } else if (H5Container.H5_PERFORMANCE.equals(action)) {
            try {
                JSONArray array = param.getJSONArray("data");
                h5Page.setH5performance(array);
            } catch (JSONException e) {
                H5Log.e(TAG, "exception", e);
            }
        } else {
    ...
    private void performBack() {
        ...
        if (backBehavior == BackBehavior.POP) {
            ...
            APWebBackForwardList list = h5WebView.copyBackForwardList();
