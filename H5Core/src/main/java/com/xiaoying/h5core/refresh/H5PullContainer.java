...
public class H5PullContainer extends FrameLayout implements OverScrollListener {
    ...
    private static int getScrollY(View apWebView) {
        ...
        if (apWebView != null && apWebView instanceof APWebView) {
            tmpWebView = (APWebView) apWebView;
            if (tmpWebView.getUnderlyingWebView() != null) {
                return tmpWebView.getUnderlyingWebView().getScrollY();
            }
        } else {
    ...
    private boolean handleTouch(MotionEvent intent) {
        ...
        if (action == MotionEvent.ACTION_MOVE) {
            ...
            int scrollY = getScrollY(contentView);
            offset = offset / 2;
    ...
    public void onOverScrolled(int deltaX, int deltaY, int scrollX, int scrollY) {
        ...
        if (getScrollY(contentView) <= 0 && deltaY < 0 && scrollY <= 0) {
            overScrolled = true;
        }
    }
